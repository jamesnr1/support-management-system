...
// Tool handlers
async function handleGetRoster(args) {
    ...
    try {
        ...
        if (teamMember) {
            filteredRoster = rosterData.filter((entry) => entry.teamMember && entry.teamMember.toLowerCase().includes(teamMember.toLowerCase()));
        }
        if (dateRange) {
...
async function handleUpdateRoster(args) {
    const { updates } = args;
    try {
        const rosterPath = path.join(WORKSPACES_ROOT, 'backend', 'roster_data.json');
        let rosterData = [];
        ...
        updates.forEach((update) => {
            const existingIndex = rosterData.findIndex((entry) => entry.teamMember === update.teamMember && entry.date === update.date);
            if (existingIndex >= 0) {
...
async function handleScheduleShift(args) {
...
async function handleGetTeamMembers(args) {
    ...
    try {
        ...
        if (role) {
            filteredMembers = teamMembers.filter((member) => member.role.toLowerCase().includes(role.toLowerCase()));
        }
        if (includeSchedule) {
...
async function handleGenerateScheduleReport(args) {
...
async function handleManageAvailability(args) {
    ...
    try {
        ...
        switch (action) {
            ...
            case 'approve_request':
                const requestIndex = requests.findIndex((req) => req.id === requestData?.requestId);
                if (requestIndex >= 0) {
